#!/usr/bin/env python3
"""15-day social-stats refresh: scrape Oracle localhost AuditPro, persist via Influo."""
...
INFLUO_BASE_URL = os.environ.get("INFLUO_BASE_URL", "https://influo.gr").rstrip("/")
...
LOG_DIR = Path(os.environ.get("SOCIAL_REFRESH_LOG_DIR", "/home/ubuntu/auditpro/logs/social-refresh"))
...
def check_sessions() -> dict[str, bool]:
    ...
    if str(health.get("status") or "").lower() not in ("operational", "ok", "healthy"):
        print(f"WARN AuditPro health: {health}")
    try:
...
def main() -> None:
    ...
    log(f"Starting Oracle social refresh → {INFLUO_BASE_URL} via {AUDITPR_URL}")
    sessions = check_sessions()
    if not sessions["instagram"] and not sessions["tiktok"]:
        start = influo_request("POST", "/api/cron/social-refresh-batch", {"action": "start"})
        run_id = start["run"]["id"]
        influo_request(
            "POST",
            "/api/cron/social-refresh-batch",
            {
                "action": "abort",
                "run_id": run_id,
                "message": "Instagram and TikTok sessions are missing on Oracle. Batch aborted.",
            },
        )
    ...
    influo_request(
        ...
        "/api/cron/social-refresh-batch",
        {
            ...
            "influencers_unapproved": unapproved_count,
            "message": f"Completed {processed}/{len(influencers)}. OK={ok_count}, unapproved={unapproved_count}.",
        },
    )
    log(f"Done. processed={processed} ok={ok_count} unapproved={unapproved_count}")
    if _LOG_FH:
        _LOG_FH.close()
